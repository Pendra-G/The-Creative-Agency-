...
function ProjectModal({ project, onClose }){
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center sm:p-4 xs:p-3 p-0 bg-ink/85 backdrop-blur-md" onClick={onClose} data-cursor>
      <div
        className="relative w-full sm:max-w-4xl bg-carbon border border-white/10 overflow-hidden flex flex-col max-h-[90vh] sm:max-h-[calc(100vh-48px)]"
      ...
      >
        ...
        <div className="p-4 xs:p-6 sm:p-10 overflow-auto" style={{ flex: '1 1 auto' }}>
          ...
          <p className="text-[10px] xs:text-[11px] uppercase tracking-[0.2em] text-paper/50 mb-4 xs:mb-6">{project.type} · {project.year} · {project.location}</p>
          <p className="text-paper/80 text-xs xs:text-base sm:text-lg leading-relaxed mb-4 xs:mb-6">{project.description}</p>
          <div className="flex flex-wrap gap-2">
            {project.scope.map(s => (
              <span key={s} className="text-[10px] xs:text-[11px] uppercase tracking-[0.2em] border border-paper/30 rounded-full px-3 py-1.5 xs:py-2 text-paper/80">{s}</span>
            ))}
          </div>
        </div>
      </div>
          ...
          {PROJECTS.map((p,i)=>(
            <button key={i} type="button" onClick={()=>setOpen(p)} data-cursor className="fw-card group block text-left w-full transition-transform duration-300 hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-white/20">
              ...
              <div className="mt-2 xs:mt-3 flex flex-col xs:flex-row items-start xs:items-end justify-between gap-2 xs:gap-4">
                ...
                <p className="text-[9px] xs:text-[10px] sm:text-[11px] uppercase tracking-[0.18em] text-paper/50 whitespace-nowrap">{p.year}</p>
              </div>
              <p className="text-[9px] xs:text-[10px] sm:text-[11px] uppercase tracking-[0.18em] text-paper/40 mt-1">{p.type} · {p.location}</p>
            </button>
          ))}
        </div>
